...
engine = create_engine('postgresql+psycopg2://postgres:w845393686@localhost:5432/SMD111')
...
connection = psycopg2.connect("host=localhost dbname=SMD111 user=postgres password=w845393686 port=5432 ")
...
for i in range(2006):
    ...
    if(i<=1919):
        list.append('Experiment1')
    else:
        list.append('Experiment2')
    list1.append(list)
col=['experiment_hub_key','experiment_businesskey','loaddatetime','recordsource','experiment_type',]
df2 = pd.DataFrame(list1,columns=col)
df2.to_sql('experiment_hub',con=engine,if_exists='append') #this writes the dataframe into the postgres databas
