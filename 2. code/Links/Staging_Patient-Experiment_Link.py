...
for i in range(1920):
        ...
        list.append(int(int(i)/int(192))+1)
        list.append(i + 1)
        list1.append(list)
for i in range(1921,2007):
        list = []
        list.append(i+1)
        list.append(datetime.now())
        list.append('Source 1')
        list.append(int(int(i-1921)%43)+11)
        list.append(i)
        list1.append(list)


# cur.execute('select sessions_hub_key from probedata_sessions_link')
# result=cur.fetchall()
# df_result=pd.DataFrame(np.array(result))
# c=np.array(df_result)
# for i in range(1,41):
#     print(c[2851*i])
#     print(c[2851*i-1])
#     print('\n')
col=['patientexperiment_link_key','loaddatetime','recordsource','patient_hub_key','experiment_hub_key']
df2 = pd.DataFrame(list1,columns=col)
df2.to_sql('patient_experiment_link',con=engine,if_exists='append') #this writes the dataframe into the postgres databas
