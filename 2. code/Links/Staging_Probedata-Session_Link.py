...
for i in range(114040):
    ...
    list.append((int(int(i)/int(2851)))%4+1)
    list.append(i + 1)
    list1.append(list)
col=['probedatasession_link_key','loaddatetime','recordsource','sessions_hub_key','probedata_hub_key']
df2 = pd.DataFrame(list1,columns=col)
df2.to_sql('probedata_sessions_link',con=engine,if_exists='append') #this writes the dataframe into the postgres databas
1.1
