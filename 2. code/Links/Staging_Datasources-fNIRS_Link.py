...
for i in range(219380,617681):
    ...
    list.append(i+1)
    list1.append(list)
col=['datasourcesfnirs_link_key','loaddatetime','recordsource','datasources_hub_key','fnirs_hub_key']
df2 = pd.DataFrame(list1,columns=col)
df2.to_sql('datasources_fnirs_link',con=engine,if_exists='append') #this writes the dataframe into the postgres database