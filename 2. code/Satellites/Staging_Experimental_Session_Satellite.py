...
def add_nts(df_list,type):
    ...
    return df_lists
list1=[]
list3=['Moto','Rest','ViMo','Viso','NormalConvo','StressedConvo']
for i in range(6):
    list = []
    ...
    list.append(list3[i])
    list1.append(list)
col=['time_stamp','sessions_hub_key','name']
df2 = pd.DataFrame(list1,columns=col)
df2.to_sql('experimental_session_satellite',con=engine,if_exists='append') #this writes the dataframe into the postgres databas
